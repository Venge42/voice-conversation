...
class WebsocketClientApp {
    ...
    constructor() {
        ...
        this.MAX_SHELLY_ERRORS = 3;
        this.shellyIP = null;
        this.currentBotLightConfig = null;
        this.autoReconnectTimer = null;
        this.isManualDisconnect = false;
        this.reconnectAttempts = 0;
        this.MAX_RECONNECT_ATTEMPTS = 120;
     ...
     * Set up light control event listeners
     */
    setupLightControlEventListeners() {
    ...
    updateColorDisplay(color) {
        ...
        if (redValue)
            redValue.textContent = Math.round(color.r * 255).toString();
        if (greenValue)
            greenValue.textContent = Math.round(color.g * 255).toString();
        if (blueValue)
            blueValue.textContent = Math.round(color.b * 255).toString();
        if (alphaValue)
            alphaValue.textContent = Math.round(color.a * 255).toString();
        // Update HEX value
   ...
   */
    async setupLightControlWebSocket() {
        try {
     ...
     */
    handleLightWebSocketMessage(message) {
     ...
     */
    runLightAnimation(botConfig, colors) {
     ...
     */
    calculateAnimatedColor(colors, elapsed) {
    ...
    calculateColorShiftFactor(elapsed, speed) {
    ...
    calculatePulseFactor(elapsed, speed, intensity) {
    ...
    calculateBreathingFactor(elapsed, speed, intensity) {
     ...
     */
    calculateComplementaryVariation(elapsed, colors) {
     ...
     */
    calculateHueShift(elapsed, colors) {
     ...
     */
    async applyLightColor(botConfig, color) {
    ...
    startAutoReconnect() {
        ...
        this.autoReconnectTimer = setTimeout(async () => {
            try {
                ...
                this.log(`❌ Auto-reconnection failed: ${error.message}`);
